...
const UpdateProfile = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await axios.get('http://localhost:5000/api/v1/mentorProfile/user/' +user?._id)
            .then(response => {
                setData(response?.data);
                setEditedData(response?.data);
                setEditedSkills(response?.data?.skills || []);
                setExpertiseData(response?.data?.expertise)
                console.log("editedData", response?.data)
            }).catch (error=> Error('Error fetching user data:', error))
        }
        fetchData();
    }, []);
    ...
    const handleImageChange = (event) => {
        ...
        if (file) {
            ...
            .then(()=>{
                setHasAvatar(true);
                setEditedData((prevData) => ({ ...prevData, avatar: imgUrl })); //comment
            })
        }
    };
    ...
    const handleUpload = async () => {
        ...
        await axios.post(`${Localhost}/api/v1/cv/upload/${user?._id}`,
            formData, { headers: { 'Content-Type': 'multipart/form-data' } }
        ).then(response=>{
            if (response.status === 200) Success('CV uploaded successfully');
            else Error('CV upload failed');
        }).catch(error=>Error('Error uploading CV:', error))
    };
    ...
    const handleEditData = (index) => { setEditingIndex(index); };

    const handleSaveData = () => { setEditingIndex(-1); };


    const handleKeyDown = (index, e) => {
        if (e.key === 'Enter') { handleSaveData(index); }
    };

    const handleDeleteRow = (index) => {
        setEditedData(prev=>({ ...prev, expertise: [...prev.expertise].splice(index, 1) }));
    };
    
    const handleAddRow = () => {
        setEditedData((prev) => ({ 
            ...prev, expertise: [ ...prev.expertise, { id: uuidv4(), name: '', from: '', to: '', }, ],
        }));
    };

    const handleSave1 = async () => {
        const updatedProfileData = { ...editedData, avatar: imgUrl };
        await axios.patch('http://localhost:5000/api/v1/mentorProfile/' + editedData?._id, updatedProfileData)
        .then(response=>{
            ...
            {data?.lookingFor === "mentee" ? (
                <div className="PersonalInfo d-flex p-1 w-100 m-auto">
                            ...
                            <div className="per-box d-flex flex-row-reverse align-items-start gap-2 m-auto p-sm-1 p-0  ">
                                ...
                                <div className="info-top d-flex flex-column align-items-center ">
                                    <div className="gryCircl">
                                        <img className="gryCircl"
                                        src={imgUrl} alt=""
                                        onClick={() => document.getElementById('fileInput').click()}/>

                                        <input type="file" id="fileInput"
                                            accept="image/*" style={{ display: 'none', height: '100px' }}
                                    ...
                                    <h6 className="p-0 m-0 text-white">{user?.role}</h6>
                                    ...
                                    <div className="form-per-med-box d-flex flex-column w-100 mt-4 p-4 justify-content-between align-items-center">
                                        ...
                                        <div className="form-per-med-box-btm5-res d-sm-none d-flex w-100 flex-column justify-content-between  ">
                                            <div className="form-per-med-left-5 flex-column d-flex  ">
                                                ...
                                                {editedData.expertise?.map((item, index) => (
                                                    <div className="exp-box5 d-flex w-100 flex-column justify-content-between align-items-end my-3 " key={index} >
                    ...
                    <div className="messages3">
                        <div className="col-lg-3  col-12  message-part mb-5 " id="message-items" >
                            ...
                            <div className="message " id="Message">
                                <div className="message-img mt-3  " style={{ width: "70px", height: "70px" }} >
                                    <img src="" alt="" style={{ width: "50px", height: "50px" }} />
                                </div>
                                <div className="message-body">
                                    <h5 className="small fw-bold">Balqees Hamdy </h5>
                                    <p className="small message-p"> econmoicc dollar is more exciting
                                    {" "}
                                        <i className="fa-solid fa-trash-can d-md-none d-block" id="trash" job="delete" ></i>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className = "message message2  " id="Message">
                                ...
                                <div className="message message2" id="Message">
                                    ...
                                    <button className="message-button  mt-3 mb-4 col-lg-11 col-11" id="message-button" onclick="sending_newmessage()" > New Message <i className="fa-solid fa-envelope"></i>
                                    </button>
                                    <div className="message_content " id="message_content">
                                        <input type="text" className="textbox ml-10" placeholder="To : " id="To" />

                                        <input className="input-text" id="bodyy" />
                                        <button type="submit" id="send_message" className="mb-3"> Send <i className="fa-solid fa-envelope"></i>
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="mdl-box-personal p-1">
                        <div className="personal-top m-auto p-sm-1 p-0">
                            ...
                            <div className="per-box d-flex flex-row-reverse align-items-start gap-2 m-auto p-sm-1 p-0  ">
                                ...
                                <div className="info-top d-flex flex-column align-items-center ">
                                    <div className="gryCircl">
                                        {/* image  */} {/* <img className="gryCircl" src={avatar} alt="Profile" /> */}
                                        <img className="gryCircl" src={imgUrl} alt="" onClick={() => document.getElementById('fileInput').click()} />
